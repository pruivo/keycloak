...
[WARNING]
====
If you prefer to use the AWS Lambda-based fencing, disable the built-in site health check with:
...
When all remote sites reappear in the cluster view _and_ their replication status is `online`, the active site transitions to RECOVERING.
...
| 1 | SUSPECTING | Failure detected, waiting one round before acting.
| 2 | UNHEALTHY | Remote site confirmed unreachable, replication disconnected.
