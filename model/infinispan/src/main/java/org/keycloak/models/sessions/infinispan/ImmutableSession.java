...
public final class ImmutableSession {
    ...
    public static Stream<UserSessionModel> copyOf(KeycloakSession session,
        ...
        var clientSessionKeys = new HashSet<UUID>();
        ...
        entityList.forEach(entity -> {
            ...
            entity.getAuthenticatedClientSessions().forEach((ignored, uuid) -> clientSessionKeys.add(uuid));
    ...
    private static void populateClientSessions(Map<String, ImmutableUserSessionModel> userSessionMap, Set<UUID> clientSessionKeys, SessionExpirationPredicates expirationPredicates, Cache<UUID, SessionEntityWrapper<AuthenticatedClientSessionEntity>> cache) {
        StreamsUtil.closing(cache.entrySet().stream()
                        .filterKeys(clientSessionKeys))
                .iterator()
                .forEachRemaining(entry -> {
                    var clientSession = entry.getValue().getEntity();
                    var userSession = userSessionMap.get(clientSession.getUserSessionId());
                    var client = expirationPredicates.realm().getClientById(clientSession.getClientId());
                    ...
                    userSession.clientSessions().put(client.getId(), copy);
                });
    }
