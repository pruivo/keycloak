...
public class InfinispanUserCacheProviderFactory implements UserCacheProviderFactory, InvalidationHandler {
    ...
    private void lazyInit(KeycloakSession session) {
        if (userCache == null) {
            synchronized (this) {
                if (userCache == null) {
                    Cache<String, CacheManager.Wrapper> cache = session.getProvider(InfinispanConnectionProvider.class).getCache(InfinispanConnectionProvider.USER_CACHE_NAME);
                    userCache = new UserCacheManager(cache);
